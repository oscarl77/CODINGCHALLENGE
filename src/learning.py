...
class FlowParser():
    ...
    def _get_readings_sum(self, file_contents, validity):
        ...
        # pattern: VALUE|DATE|STATUS of which we want VALUE
        for line in file_contents:
            match = re.search(valid_readings_pattern, line)
            # match corresponds to a reading value
            if match: 
                meter_readings_sum += float(match.group(1))
    ...
    def _get_newest_meter_reading(self, file_contents):
        ...
        Returns:
            float: most recent meter reading in the file.
        """
